	...
	methods: {
		...
		getdata() {
			axios
				...
				.then((res) => {
					console.log(res);
					this.ites = res.data.data1;
					// console.log(this.ites)
					for (let i = 0; i < 25; i++) {
						for (let j = 0; j < 8; j++) {
							// this.desserts["id" + j] = this.ites[i][j];
							this.$set(this.desserts, this.desserts.id0,this.ites[i][j])
							console.log(this.desserts.id0)
							console.log(this.ites[i][j])
